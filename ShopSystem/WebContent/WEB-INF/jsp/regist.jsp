...
<link href="${pageContext.request.contextPath}/css/error.css" rel="stylesheet" type="text/css"/>
		...
		<link rel="stylesheet" href="${pageContext.request.contextPath}/js/datepick/jquery.datepick.css" type="text/css">
		<script type="text/javascript" src="${pageContext.request.contextPath}/js/datepick/jquery.datepick.js"></script>
		<script type="text/javascript" src="${pageContext.request.contextPath}/js/datepick/jquery.datepick-zh-CN.js"></script>
		<!-- 表单校验插件 -->
		<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery-validate/jquery.validate.min.js"></script>
			...
			$('#registerForm').validate({
				...
				rules:{
					username:{
						...
						maxlength:20,
						 remote:{
							url: "${pageContext.request.contextPath}/user_checkUserName.action",     //后台处理程序
						    type: "post",               //数据发送方式
					...
					email:{
						...
						email:true,
						remote:{
								url: "${pageContext.request.contextPath}/user_checkEmail.action",     //后台处理程序
							    type: "post",               //数据发送方式
							    dataType: "json",           //接受数据格式   
							    data: {                     //要传递的数据
							        username: function() {
				 ...
				 messages: {
					 username:{
							required: "请输入用户名",
							minlength:"用户名长度必须在3-20位",
							maxlength:"用户名长度必须在3-20位",
							remote:"用户名已存在",
							isUsername:"用户名必须只能包含字母数字以及下划线"
						},
						password:{
							required: "请输入密码",
							minlength:"密码长度必须在3-15位",
							maxlength:"密码长度必须在3-15位",
							equalTo: "两次密码不一致",
						},
						rePassword:{
							required: "请重复输入密码",
				 ...
				 errorPlacement: function(error, element) {
					//当标签为radio的时候,需要获取它父亲的父亲，然后再父亲的结尾加上错误信息 
					if(element.is(':radio')){
					...
					if(element.attr('name')=='password'||element.attr('name')=='rePassword'){
						$('#password').val('');
						$("#rePassword").val('');
					}
					...
					<form id="registerForm"  method="post" action="${pageContext.request.contextPath}/user_regist.action">
						<table>
							<tbody><tr>
								...
								<td>
									<input type="text" id="username" name="username" class="text" maxlength="20" value="${param.username }" /> <label class="error"><s:property value="fieldErrors.username[0]"/></label>
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									<input type="password" id="password" name="password" class="text" maxlength="20" autocomplete="off"> <label class="error"><s:property value="fieldErrors.password[0]"/></label>
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									<input type="password" id="rePassword" name="rePassword" class="text" maxlength="20" autocomplete="off" > <label class="error"><s:property value="fieldErrors.rePassword[0]"/></label>
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									<input type="text" id="email" name="email" class="text" maxlength="200" value="${param.email }"> <label class="error"><s:property value="fieldErrors.email[0]"/></label>
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									<input type="text" id="phone" name="phone" class="text" maxlength="200" value="${param.phone }"> <label class="error"><s:property value="fieldErrors.phone[0]"/></label>
								</td>
							</tr>
									...
									<tr>
										...
										<td>
												<input type="text" id="name" name="name" class="text" maxlength="200" value="${param.name }"> <label class="error"><s:property value="fieldErrors.name[0]"/></label>
										</td>
									</tr>
									<tr>
										<th>
										...
										<td>
												<span class="fieldSet">
														<label>
															<input type="radio" id="sex" name="sex" value="1" <s:if test='#parameters.sex[0]=="1" '>checked="checked"</s:if>>男
														</label>
														<label>
															<input type="radio" id="sex" name="sex" value="0" <s:if test='#parameters.sex[0]=="0" '>checked="checked"</s:if> >女
														</label>
															<label class="error"><s:property value="fieldErrors.sex[0]"/></label>
												</span>
										</td>
									</tr>
									<tr>
										...
										<td>
												<input id="birthday" type="text" name="birthday" class="text" readonly="readonly" value="${param.birthday}"> <label class="error"><s:property value="fieldErrors.birthday[0]"/></label>
										</td>
									</tr>
									<tr>
										<th>
										...
										<td>
												<textarea  style="vertical-align: middle; color: black" cols="30" rows="3" name="addr" id="addr"  >${param.addr}</textarea>  <label class="error"><s:property value="fieldErrors.addr[0]"/></label>
										</td>
									</tr>
								<!-- 	
								<tr>
